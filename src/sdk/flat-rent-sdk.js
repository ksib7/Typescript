...
export class FlatRentSdk {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        flats = flats.filter((flat) =>
          this._areAllDatesAvailable(flat, dateRange)
        );

        flats = flats.map((flat) =>
          this._formatFlatObject(flat, dateRange.length - 1)
        );
  ...
  _areAllDatesAvailable(flat, dateRange) {
    return dateRange.every(
      (date) => !flat.bookedDates.includes(date.getTime())
    );
  }
  ...
  _formatFlatObject(flat, nightNumber) {
    ...
    formattedFlat.photos = formattedFlat.photos.map(
